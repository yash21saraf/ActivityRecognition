...
from src.networks.network_base import max_pool, upsample, inverted_bottleneck, separable_conv, convb, is_trainable
...
def build_network(input, trainable, scale = 1):
    ...
    with tf.variable_scope("Convolutional_Pose_Machine"):
        ...
        for stage_number in range(STAGE_NUM):
            ...
            prev = _
            cpm_out = upsample(_, int(8/scale), "stage_%d_out" % stage_number)
            l2s.append(cpm_out)
