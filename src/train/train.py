...
import numpy as np
import configparser
import src.dataloaders.dataset

from datetime import datetime

from src.dataloaders.dataset import get_train_dataset_pipeline, get_valid_dataset_pipeline
from src.networks.networks import get_network
from src.dataloaders.dataset_prepare import CocoPose
from src.dataloaders.dataset_augment import set_network_input_wh, set_network_scale
...
def get_loss_and_output(model, batchsize, input_image, input_heat, reuse_variables=None, scale = 1):
    losses = []

    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
        _, pred_heatmaps_all = get_network(model, input_image, True, scale  )
            ...
            with tf.device("/cpu:0"):
                with tf.name_scope("CPU_0"):
                    loss, last_heat_loss, pred_heat = get_loss_and_output(params['model'], params['batchsize'],
                                                                          input_image, input_heat, reuse_variable,params['scale'])
                    reuse_variable = True
            ...
            for i in range(params['gpus']):
                with tf.device("/gpu:%d" % i):
                    with tf.name_scope("GPU_%d" % i):
                        loss, last_heat_loss, pred_heat = get_loss_and_output(params['model'], params['batchsize'], input_image, input_heat, reuse_variable, params['scale'])
                        reuse_variable = True
                        grads = opt.compute_gradients(loss)
                        tower_grads.append(grads)
